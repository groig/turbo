defmodule TurboWeb.ConfirmationControllerTest do
  ...
  describe "POST /confirm/:token" do
    test "confirms the given token once", %{conn: conn, user: user} do
      ...
      conn = post(conn, Routes.confirmation_path(conn, :update, token)) |> doc
    ...
    test "does not confirm email with invalid token", %{conn: conn, user: user} do
      conn = post(conn, Routes.confirmation_path(conn, :update, "oops")) |> doc
      assert conn.status == 410
      assert conn.resp_body =~ "User confirmation code is invalid or it has expired"
