defmodule Turbo.AccountsTest do
  ...
  describe "get_user_by_email_or_phone_and_password/2" do
    test "does not return the user if the email does not exist" do
      refute Accounts.get_user_by_email_or_phone_and_password(
               "unknown@example.com",
               "hello world!"
             )
    end
    ...
    test "does not return the user if the password is not valid" do
      user = user_fixture()
      refute Accounts.get_user_by_email_or_phone_and_password(user.email, "invalid")
    end
    ...
    test "returns the user if the email and password are valid" do
      ...
      assert %User{id: ^id} =
               Accounts.get_user_by_email_or_phone_and_password(user.email, valid_user_password())
    end
  end
  ...
  describe "change_user_registration/2" do
    test "returns a changeset" do
      ...
      assert changeset.required == [:password, :email, :type, :name, :phone]
    end
  ...
  describe "generate_user_token/1" do
    setup do
      %{user: user_fixture()}
    end
  ...
  describe "delete_token/1" do
    test "deletes the token" do
      user = user_fixture()
      token = Accounts.generate_user_token(user)
      assert Accounts.delete_token(token) == :ok
